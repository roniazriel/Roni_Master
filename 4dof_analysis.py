...
'''Load data'''
# dataset = pd.read_csv('4_dof_results.csv')
...
# problematic = best_4dof_arms.loc[best_4dof_arms['Manipulability - mu'] == -1]
# problematic_arms = problematic["Arm_ID"].unique()
# problematic_arms = [arm+'.urdf.xacro' for arm in problematic_arms]
# print(problematic_arms)
# import os
# for filename in os.listdir("/home/ar1/catkin_ws/src/sock-puppet/man_gazebo/tested_arms/"):
#     print(filename)
#     if filename in problematic_arms:
#         os.rename("/home/ar1/catkin_ws/src/sock-puppet/man_gazebo/tested_arms/"+filename, "/home/ar1/catkin_ws/src/sock-puppet/man_gazebo/best_tested_arms/" + filename)
#         print(filename)

#
# '''Group by arm id'''
# best_4dof_arms = pd.read_csv('best_4dof_results.csv')
# best_4dof_arms = best_4dof_arms.drop(columns='Unnamed: 0')
# print(best_4dof_arms.keys())
# grouped_best4dof_data = best_4dof_arms.groupby(['Arm ID']).agg(
...
#
# # good_arms = grouped_best4dof_data.loc[grouped_best4dof_data['Success_Rates'] == 0.6]
# # print(len(good_arms))
#
# grouped_best4dof_data = grouped_best4dof_data.drop(
#     grouped_best4dof_data[grouped_best4dof_data.Success_Rates < 1].index).reset_index()
# with pd.option_context('display.max_rows', None,
#                        'display.max_columns', None,
#                        'display.precision', 3):
#     print(grouped_best4dof_data)
...
# new = grouped_best4dof_data["Arm ID"].str.split("_", expand=True)
...
#
# grouped_best4dof_data = grouped_best4dof_data.drop(['Arm ID'], axis=1)
...
# #                                                       'Joint1 axis', 'Joint2 type', 'Joint2 axis',
# #                                                       'Joint3 type', 'Joint3 axis', 'Joint4 type',
# #                                                       'Joint4 axis']).count().reset_index()
# #
# # best_4dof_joint_conf = best_4dof_joint_conf.drop(
# #     columns=['Success_Rates', 'Min_Manipulability', 'Max_Manipulability', 'Manipulability_Rates',
# #              'MaxSum_Mid_joint_proximity',
# #              'Link1 length', 'Link2 length', 'Link3 length', 'Link4 length'])
